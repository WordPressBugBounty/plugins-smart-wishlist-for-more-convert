...
if ( ! class_exists( 'WLFMC_Analytics_Admin_Demo' ) ) {
	...
	class WLFMC_Analytics_Admin_Demo {
		...
		public function __construct() {
			add_filter( 'wlfmc_admin_menus', array( $this, 'add_admin_menu' ), 30, 2 );
		}
		...
		public function add_admin_menu( $admin_menus, $parent_slug ) {
			...
			if ( ! defined( 'MC_WLFMC_PREMIUM' ) ) {
				$new_menus['analytics'] = add_submenu_page(
					$parent_slug,
			...
			if ( ! defined( 'MC_WLFMC_PREMIUM' ) ) {
				$new_menus['features'] = add_submenu_page(
					$parent_slug,
			...
			if ( ! defined( 'MCM_MESSAGE_VERSION' ) ) {
				$new_menus['message_features'] = add_submenu_page(
					$parent_slug,
					__( 'Message', 'wc-wlfmc-wishlist' ),
					__( 'Message', 'wc-wlfmc-wishlist' ),
					apply_filters( 'wlfmc_capability', 'manage_options' ),
					'mc-message-features',
					array( $this, 'show_message_features' )
				);
			}
			if ( ! defined( 'WLFMC_PLUS_VERSION' ) ) {
				$new_menus['afs_features'] = add_submenu_page(
					$parent_slug,
					__( 'Ask for Estimate', 'wc-wlfmc-wishlist' ),
					__( 'Ask for Estimate', 'wc-wlfmc-wishlist' ),
					apply_filters( 'wlfmc_capability', 'manage_options' ),
					'mc-afs-features',
					array( $this, 'show_afs_features' )
				);
			}
			if ( ! defined( 'MC_WLFMC_PREMIUM' ) ) {
				$new_menus['premium'] = add_submenu_page(
					$parent_slug,
			...
			// Insert new menus at the correct position.
			if ( false !== $index ) {
				...
				// If 'email-automations' is not found, append new menus.
				$admin_menus = array_merge( $admin_menus, $new_menus );
			}
			if ( ! defined( 'MC_WLFMC_PREMIUM' ) && isset( $admin_menus['dashboard'] ) ) {
		 ...
		 * @return void
		 */
		public function show_message_features() {
			...
			$fields = new MCT_Fields(
				array(
					...
					'desc'           => __( 'Gather leads and boost your sales with attention-grabbing lists for potential customers.', 'wc-wlfmc-wishlist' ),
					'logo'           => '<img src="' . MC_WLFMC_URL . 'assets/backend/images/logo.svg" width="45" height="40"  alt="logo"/>',
					'id'             => 'wlfmc_message_features_page',
					'header_buttons' => wlfmc_get_admin_header_buttons(),
					'header_menu'    => wlfmc_get_admin_header_menu(),
					'type'           => 'class-type',
					'class_array'    => array( $this, 'display_message_features' ),
				)
			);
			$fields->output();
		}
		 ...
		 * @return void
		 */
		public function show_afs_features() {
			...
			$fields = new MCT_Fields(
				array(
					...
					'desc'           => __( 'Gather leads and boost your sales with attention-grabbing lists for potential customers.', 'wc-wlfmc-wishlist' ),
					'logo'           => '<img src="' . MC_WLFMC_URL . 'assets/backend/images/logo.svg" width="45" height="40"  alt="logo"/>',
					'id'             => 'wlfmc_afs_features_page',
					'header_buttons' => wlfmc_get_admin_header_buttons(),
					'header_menu'    => wlfmc_get_admin_header_menu(),
					'type'           => 'class-type',
					'class_array'    => array( $this, 'display_afs_features' ),
				)
			);
			$fields->output();
		}
				...
				<?php foreach ( $articles as $article ) : ?>
					<div class="mct-article mar-bot-20">
						...
						<div class="container">
							...
							<?php endforeach; ?>
							<?php
							$column_count = count( $article['columns'] );
							if ( $column_count < 5 ) {
								for ( $i = 0; $i < 5 - $column_count;$i++ ) {
									echo '<div></div>';
								}
							}
							?>
		 ...
		 * @return void
		 */
		public function display_afs_features() {
			$article = array(
				'title'        => __( 'Key Features', 'wc-wlfmc-wishlist' ),
				'descriptions' => __( 'Key Features of MoreConvert Ask for Estimate Plugin for WooCommerce.', 'wc-wlfmc-wishlist' ),
				'columns'      => array(
					array(
						'image' => 'feature-1.png',
						'title' => __( 'One-click Estimate Button', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Adds an "Ask for Estimate" button at the bottom of any wishlist or other lists pages.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-2.png',
						'title' => __( 'Works Across All List Types', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Fully integrated with MoreConvert Pro features -- including wishlist,Multi-lists, Waitlists and more.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-3.png',
						'title' => __( 'Smart Popup With Optional Notes', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'A sleek popup lets users and extra information before submitting their request.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-4.png',
						'title' => __( 'Editable Button Styles', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Customize button size, color , border and font to match your theme.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-5.png',
						'title' => __( 'Flexible Display Options', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Choose who sees the button: all users, logged-in users only or restricted.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-6.png',
						'title' => __( 'Optional Tooltip & Notes', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Add additional messaging or notes for admins right inside the request popup.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-7.png',
						'title' => __( 'Send Requests to Admin', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Automatically sends estimate requests to your chosen email address.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-13.png',
						'title' => __( 'Add Multiple Recipients (CC)', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Notify your sales or support team instantly.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-8.png',
						'title' => __( 'Flexible Email Templates', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Let admins receive estimate requests in the style that fits best.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-10.png',
						'title' => __( 'Multilingual Support', 'wc-wlfmc-wishlist' ),
						'desc'  => __( ' Works with WPML, Polylang, and other popular translation plugins.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-11.png',
						'title' => __( 'Built For Speed', 'wc-wlfmc-wishlist' ),
						'desc'  => __( 'Lightweight, fast, and clean-coded to avoid performance issues.', 'wc-wlfmc-wishlist' ),
					),
					array(
						'image' => 'feature-12.png',
						'title' => __( 'Moreconvert Pro Integration', 'wc-wlfmc-wishlist' ),
						'desc'  => __( ' This plugin works in sync with MoreConvert Pro features like Wishlist, Waitlist, and other tools.', 'wc-wlfmc-wishlist' ),
					),
				),
			);
			WLFMC_Admin_Notice()->styles();
			?>
			<div id="wlfmc-features">
				<div id="wlfmc-premium-admin-notice" class="mct-article wlfmc-orange-article mar-bot-20">
					<h2><?php echo wp_kses_post( __( 'One-Click Price Requests for Your WooCommerce Lists', 'wc-wlfmc-wishlist' ) ); ?></h2>
					<p class="mar-bot-20">
						<?php esc_html_e( 'Enable customers to “Ask for Estimate” from any list — Wishlist, Waitlist, Multi-List, and more — all in one click.', 'wc-wlfmc-wishlist' ); ?>
					</p>
					<div class="d-flex wlfmc-message-features">
						<div class="content-wrapper">
							<div class="d-flex mct-article gap-5 f-wrap mar-bot-20" style="background:transparent !important">
								<h3>
									<?php echo wp_kses_post( __( 'How It Works?', 'wc-wlfmc-wishlist' ) ); ?>
								</h3>
								<p class="mb-2">
									<?php esc_html_e( 'Once enabled, a clean Ask for Estimate button appears under your product lists. Customers can request a price estimate — with optional notes — in just one click. Requests are sent directly to the store admin via email.', 'wc-wlfmc-wishlist' ); ?>
								</p>
								<div class="d-flex f-row f-wrap gap-5">
									<div class="d-flex f-column  gap-5">
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/afs-features/puzzle.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Compatible with all MoreConvert list types', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/afs-features/envelope.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Optional notes & CC recipients', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
									</div>
									<div class="d-flex f-column  gap-5">
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/afs-features/edit.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Customize the button & email content', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/afs-features/tick.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Easy setup. No bloat.', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
									</div>
								</div>
							</div>
							<div class="d-flex gap-10">
								<a href="https://moreconvert.com/9l9c" target="_blank" class="btn-primary black-btn" ><?php esc_attr_e( 'Go To Plugin Details', 'wc-wlfmc-wishlist' ); ?> <span class="dashicons dashicons-arrow-<?php echo is_rtl() ? 'left' : 'right'; ?>-alt"></span></a>
								<a href="https://moreconvert.com/l3xq" target="_blank" class="btn-secondary "><?php esc_attr_e( 'Try the Demo', 'wc-wlfmc-wishlist' ); ?></a>
							</div>
						</div>
						<div class="iframe-wrapper">
							<iframe width="560" height="315" src="https://www.youtube.com/embed/YlS52qzAWEM?si=5mnEVrGUqVqrokYu" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
						</div>
					</div>
				</div>
				<div class="mct-article mar-bot-20">
					...
					<div class="container">
						<?php foreach ( $article['columns'] as $column ) : ?>
							<div class="column d-flex f-column mct-article">
								<div class="d-flex f-column center-align">
									<img class="thumbnail mar-bot-20" alt="" src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/afs-features/<?php echo esc_attr( $column['image'] ); ?>" />
									<strong><?php echo wp_kses_post( $column['title'] ); ?></strong>
									<p><?php echo wp_kses_post( $column['desc'] ); ?></p>
								</div>
								<a class="btn-primary d-flex gap-5 f-center <?php echo isset( $column['disabled'] ) && wlfmc_is_true( $column['disabled'] ) ? 'disabled' : ''; ?>" href="https://moreconvert.com/srge" target="_blank">
									<img width="" alt="" src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/wizard/crown.svg" />
									<span><?php esc_attr_e( 'Unlock this Feature', 'wc-wlfmc-wishlist' ); ?></span>
								</a>
							</div>
						<?php endforeach; ?>
						<?php
			...
			<style>
				...
				@media( min-width: 992px ) {
					...
					.wlfmc-message-features .content-wrapper {
		 ...
		 * @return void
		 */
		public function display_message_features() {
			$article = array(
				'title'        => __( 'Key Features', 'wc-wlfmc-wishlist' ),
				'descriptions' => __( 'Key Features of MoreConvert Ask for Estimate Plugin for WooCommerce.', 'wc-wlfmc-wishlist' ),
				'columns'      => array(
					array(
						'image'    => 'feature-1.png',
						'title'    => __( 'Waitlist Product Alerts', 'wc-wlfmc-wishlist' ),
						'desc'     => __( ' Inform users when a product they’ve waitlisted is back in stock, discounted, or price-dropped.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-2.png',
						'title'    => __( 'Order Status Messages', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Notify users when their order status changes — from processing to shipped, completed, or cancelled.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-3.png',
						'title'    => __( 'Coupon Notifications', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Show newly published discount codes — automatically, right in the message center.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-4.png',
						'title'    => __( 'Comment & Review Updates', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Let customers know when their product reviews or blog comments are approved or replied to.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-5.png',
						'title'    => __( 'Fully Editable Texts', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Personalize all message content to match your brand’s tone and voice.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-6.png',
						'title'    => __( 'Visibility & Display Rules', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Activate or deactivate messages for waitlist items, order status, coupons, and comments with a single click.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-7.png',
						'title'    => __( 'Live Notification Counter', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Display unread message counts in your site’s menu, header, or anywhere you prefer.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
					),
					array(
						'image'    => 'feature-11.png',
					...
					array(
						'image'    => '',
						'title'    => __( 'Instant Notification Alerts', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Users see a real-time alert when they receive a new message from the admin.', 'wc-wlfmc-wishlist' ),
						'activate' => false,
						'disabled' => true,
					),
					array(
						'image'    => '',
						'title'    => __( 'Personalized Support/Updates', 'wc-wlfmc-wishlist' ),
						'desc'     => __( 'Send tailored responses, request feedback or follow up on orders --- without relying on email', 'wc-wlfmc-wishlist' ),
						'activate' => false,
						'disabled' => true,
					),
				),
			...
			<div id="wlfmc-features">
				<div id="wlfmc-premium-admin-notice" class="mct-article wlfmc-orange-article mar-bot-20">
					<h2><?php echo wp_kses_post( __( 'Smart, On-Site Messaging for WooCommerce Customers', 'wc-wlfmc-wishlist' ) ); ?></h2>
					<p class="mar-bot-20">
						<?php esc_html_e( 'Moreconvert Message Hub adds a clean, user-focused message center inside your WooCommerce "My Account" area..', 'wc-wlfmc-wishlist' ); ?>
					</p>
					<div class="d-flex wlfmc-message-features">
						<div class="content-wrapper">
							<div class="d-flex mct-article gap-5 f-wrap mar-bot-20" style="background:transparent !important">
								...
								<p class="mb-2">
									<?php esc_html_e( 'Once installed,this plugin adds a new "Messages" tab to the WooCommerce My Account page. Users automatically receive messages when key events happen -- related to orders, products, waitlist items, coupons or reviews', 'wc-wlfmc-wishlist' ); ?>
								</p>
								<div class="d-flex f-row f-wrap gap-5">
									<div class="d-flex f-column  gap-5">
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/message-features/linked.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Linked to the relevant event or product', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/message-features/view.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Displayed inside the site (not via email)', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
									</div>
									<div class="d-flex f-column  gap-5">
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/message-features/list.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Organized, timestamped and visible anytime', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
										<div class="d-flex f-center gap-5">
											<img src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/message-features/edit.svg" width="40" height="40" alt="features" />
											<strong><?php esc_attr_e( 'Styled and worded based on your settings', 'wc-wlfmc-wishlist' ); ?></strong>
										</div>
									</div>
								</div>
							</div>
							<div class="d-flex gap-10">
								<a href="https://moreconvert.com/o1cv" target="_blank" class="btn-primary black-btn" ><?php esc_attr_e( 'Go To Plugin Details', 'wc-wlfmc-wishlist' ); ?> <span class="dashicons dashicons-arrow-<?php echo is_rtl() ? 'left' : 'right'; ?>-alt"></span></a>
								<a href="https://moreconvert.com/03c5" target="_blank" class="btn-secondary "><?php esc_attr_e( 'Try the Demo', 'wc-wlfmc-wishlist' ); ?></a>
							</div>
						</div>
						<div class="iframe-wrapper">
							<iframe width="560" height="315" src="https://www.youtube.com/embed/RZ7BYatUh30?si=RpwPpDCOGltR4vO5" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
						</div>
					</div>
				</div>
				<div class="mct-article mar-bot-20">
					...
					<div class="container">
						<?php foreach ( $article['columns'] as $column ) : ?>
							<div class="column d-flex f-column mct-article">
								<div class="d-flex f-column center-align">
									<?php if ( ! $column['image'] ) : ?>
										<svg class="thumbnail mar-bot-20" style="width: 100%; max-width: 100%;height: auto;border-radius:6px" width="405" height="405" viewBox="0 0 405 405" xmlns="http://www.w3.org/2000/svg">
											<rect width="405" height="405" fill="#ededed"/>
											<text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" fill="#333" font-size="22" font-weight="bold" >COMING SOON</text>
										</svg>
									<?php else : ?>
										<img class="thumbnail mar-bot-20" alt="" src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/message-features/<?php echo esc_attr( $column['image'] ); ?>" />
									<?php endif; ?>
									<strong><?php echo wp_kses_post( $column['title'] ); ?></strong>
									<p><?php echo wp_kses_post( $column['desc'] ); ?></p>
								</div>

								<a class="btn-primary d-flex gap-5 f-center <?php echo isset( $column['disabled'] ) && wlfmc_is_true( $column['disabled'] ) ? 'disabled' : ''; ?>" href="https://moreconvert.com/hzif" target="_blank">
									<img width="" alt="" src="<?php echo esc_url( MC_WLFMC_URL ); ?>assets/backend/images/wizard/crown.svg" />
									<span><?php esc_attr_e( 'Unlock this Feature', 'wc-wlfmc-wishlist' ); ?></span>
								</a>
							</div>
		...
		public function display_overview() {
			...
			$top_products    = $wpdb->get_results(
				"
                                    ...
                                    GROUP BY items.prod_id ORDER BY COUNT( DISTINCT items.customer_id ) DESC LIMIT 3",
				'ARRAY_A'
			);
								...
								<div class="d-flex">
									...
									<div class="wlfmc-content">
										<p class="d-flex gap-10 f-center">
											...
											<strong>
												<?php
												// translators: %1$d is the number of potential customers, %2$s is the product name with a link.
												echo wp_kses_post( sprintf( __( '%1$d Potential customers for %2$s', 'wc-wlfmc-wishlist' ), $p3_count, '<a href="' . esc_url( $product_3->get_permalink() ) . '" style="font-style:italic;color:#FD5D00;">' . wp_kses_post( is_callable( array( $product_3, 'get_name' ) ) ? $product_3->get_name() : $product_3->get_title() ) . '</a>' ) );
												?>
											</strong>
										</p>
										<p class="description" style="font-weight:500">
											<?php
											// translators: %d is the number of potential customers who have added the product to their wishlist.
											echo esc_attr( sprintf( __( 'Create an automation for the potential %d customers who have added the specific product to their wishlist. Invite them to buy it right now with different ideas, such as a contest, on-sale automation, or even a webinar for them, with a special coupon provided at the end. Start increasing your revenue and don\'t lose this opportunity', 'wc-wlfmc-wishlist' ), $p3_count ) );
											?>
										</p>
										<p>
							...
							<?php if ( $product_1 && $p1_count > 1 ) : ?>
								<div class="mct-article mar-bot-20 wlfmc-style-1">
									<div class="d-flex gap-20 f-center">
										...
										<?php
										// translators: %s is the number of positive reviews lost, displayed in a styled HTML heading.
										echo wp_kses_post( sprintf( __( '%s <strong>Positive Review Lost!</strong>', 'wc-wlfmc-wishlist' ), '<h1 style="color:#FF5F5F;font-size: 44px;padding:0"><strong>' . intval( $p1_count / 3 ) . '</strong></h1>' ) );
										?>
									</div>
									<p style="font-weight:400">
										<?php
										// translators: %s is the product name with a styled HTML link.
										echo wp_kses_post( sprintf( __( 'Lost Potential Review for %s', 'wc-wlfmc-wishlist' ), '<a href="' . esc_url( $product_1->get_permalink() ) . '" style="font-style:italic;color:#FF5F5F;">' . wp_kses_post( is_callable( array( $product_1, 'get_name' ) ) ? $product_1->get_name() : $product_1->get_title() ) . '</a>' ) );
										?>
									</p>

									<p class="description mar-bot-20" style="font-weight:500">
										<?php
										// translators: %1$d is the number of users with the product on their wishlist, %2$d is the estimated number of positive reviews possible from an email campaign.
										echo esc_attr( sprintf( __( 'With %1$d users who have this product on their wishlist, you can gain at least %2$d positive reviews by running a email campaign for its users. Set conditions for a big deal for all those who send the best reviews under this product.', 'wc-wlfmc-wishlist' ), $p1_count, intval( $p1_count / 3 ) ) );
										?>
									</p>
							...
							<?php if ( $outofstock_products ) : ?>
								<div class="mct-article mar-bot-20 wlfmc-style-2">
									<div class="d-flex f-column f-center">
										...
										<p class="description mar-bot-20 center-align" style="max-width:300px">
											<?php
											// translators: %d is the number of out-of-stock products.
											echo esc_attr( sprintf( __( 'With %d out-of-stock products, you must immediately convert potential customers on your product pages into leads and boost sales by promoting similar or alternative available products to them.', 'wc-wlfmc-wishlist' ), $outofstock_products ) );
											?>
										</p>
										<p>
							...
							<?php if ( $product_2 ) : ?>
								<div class="mct-article mar-bot-20 wlfmc-style-3">
									...
									<p style="font-weight:500">
										<?php
										// translators: %s is the product name with a styled HTML link.
										echo wp_kses_post( sprintf( __( 'Build a newsletter for: %s', 'wc-wlfmc-wishlist' ), '<a href="' . esc_url( $product_2->get_permalink() ) . '" style="font-style:italic;color:#5EC4FF;">' . wp_kses_post( is_callable( array( $product_2, 'get_name' ) ) ? $product_2->get_name() : $product_2->get_title() ) . '</a>' ) );
										?>
									</p>
									<p class="description mar-bot-20">
										<?php
										// translators: %d is the number of potential customers who have added the product to their wishlist.
			...
			<?php if ( $product_2 || $product_3 ) : ?>
				...
				<div id="modal_automation" class="mct-modal modal_automation" style="display:none">
					...
					<div class="modal-wrapper modal-transition <?php echo esc_attr( $modal['class'] ); ?>">
